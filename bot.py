...
async def cmd_burmalda(message: Message) -> None:
    ...
    try:
        ...
        logger.info(f"Burmalda dice value: {value}")
        
        # Проверяем комбинации
        if value == 64:
            # Это может быть как три семёрки, так и три BAR
            # По умолчанию считаем это джекпотом с тремя семёрками
            ...
            # Устанавливаем флаг, что выпали три семёрки (или BAR)
            burmalda_sevens_triggered = True
        elif value == 0:
            # Три BAR (в некоторых версиях это отдельное значение)
            text = "Три бара. О, как впечатляюще. Ты почти выиграл джекпот. Почти. Но 'почти' считается только в игре в поддавки."
            # BAR тоже считаем как выигрышную комбинацию, но не меняем вступление
            # Если хотите, чтобы BAR тоже менял вступление - раскомментируйте строку ниже
            # burmalda_sevens_triggered = True
            ...
            text = "В последний раз — и ты снова проиграл. Предсказуемо."
